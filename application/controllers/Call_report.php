...
class Call_report extends CI_Controller {
    ...
    public function call_log(){
        ...
        $data['content'] = $this->load->view('telephony/call_log', $data, true);
        $this->load->view('layout/main_wrapper', $data);		
	}
    public function index(){
		if(user_access(222) && user_access(430)){
			redirect('call_report/call_log');
		}
	...
	public function call_log_datatable(){
		$this->load->model('call_log_model');
		$result = $this->call_log_model->get_datatables();
		$data = array();
		if(!empty($result)){
			foreach($result as $key=>$value){
				...
				$row[] = $value->agent_no;
				$row[] = "<audio controls>
						<source src=".$value->recording.">						
			  			</audio>";
				$row[] = $value->called_no;
				$row[] = $value->duration;
				$row[] = $value->call_status;
				$row[] = $value->call_direction;
				$row[] = $value->call_time;
		...
		$output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->call_log_model->count_all(),
            "recordsFiltered" => $this->call_log_model->count_filtered(),
            "data" => $data,
        );
