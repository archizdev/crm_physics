...
class Client extends CI_Controller {
    ...
    public function create_newcontact() 
    {
        ...
        if (!empty($_POST)) {
            ...
            $contact_code = $account_id = '';
            if(!empty($enq->row()->company)){                
                $account_id = $enq->row()->company;                
                $this->db->where('account_id',$account_id);
                $this->db->order_by('cc_id','desc');
                $crow = $this->db->get('tbl_client_contacts')->row_array();
                //print_r($crow);
                if(!empty($crow['contact_code'])){
                    //$next_char = substr($crow['contact_code'],-1)+1;
                    $next_char = substr($crow['contact_code'],-1);
                    $next_char++;
                    $contact_code = substr_replace($crow['contact_code'] ,$next_char,-1);
                }else{
                    $this->db->select('comp_code');
                    $this->db->where('id',$account_id);
                    $accrow = $this->db->get('tbl_company')->row_array();
                    if(!empty($accrow['comp_code'])){
                        $contact_code = $accrow['comp_code'].'A';
                    }else{
                        $contact_code = '';
                    }
                }
   ...
   // print_r($c->enq_ids); exit();
    ...
    public function updateclient($enquiry_id = null) {         
        ...
        if(!empty($company)){
            $company_row = $this->db->where('company_name',$company)->get('tbl_company')->row();
            if(!empty($company_row)){
                $company = $company_row->id;
            }
            else{
                $prefix = get_sys_parameter('account_code_prefix','COMPANY_SETTING');
                if(!$prefix){
                    $prefix = 'CUST';
                }
                $new_company = array(
                ...
                $cust_code = $prefix.$company;                    
                $this->db->where('tbl_company.comp_id',$this->session->companey_id);
                $this->db->where('tbl_company.id',$company);
                $this->db->set('tbl_company.comp_code',$cust_code);
        ...
        $this->db->set('enquiry_subsource',$this->input->post('product_country'));
...
$user_list = $this->db->select('CONCAT(s_display_name," ",last_name) emp_name,designation')->from('tbl_admin')->where('companey_id',$this->session->companey_id)->get()->result();
    ...
    public function checkmobile(){        
        $this->session->set_userdata('call_res',$this->input->post('res'));        
        if (!empty($this->input->post('proc'))) {
            $this->db->where('product_id',$this->input->post('proc'));    
        }
        if($this->input->post('type') == 'email'){
            $field = 'Email Id';
            $totp = $this->db->where("email", $this->input->post("mobileno", true))->from("enquiry")->get();        
        }else{
            $field = 'Mobile No';
            $totp = $this->db->where("phone", $this->input->post("mobileno", true))->from("enquiry")->get();        
        }
        $c    = $totp->num_rows();
        $mob =   $this->input->post("mobileno", true);
		$enq=0;
		if($c > 0){	            
            $msg = '<span class="badge badge-danger" style ="background:red;padding:2px;">'.$field.' already exists </span>';
            $status = 0;
            $totp = $totp->result_array();            
            $t = ''; 
            if(!empty($totp)){
                ...
                $t .= '<table class="table table-bordered">';
                    ...
                    foreach ($totp as $key => $value) { 
                        ...
                        if($value['drop_status'] > 0){
                            $t .= '<tr style="background:red;color:white;">';               
                        }else{
                            $t .= '<tr>';               
                        }
                        ...
                        $t .= '<td>'.$value['phone'].'</td>';                                                            
                        $t .= '<td>'.$value['email'].'</td>';                                                            
                        $t .= '<td>'.$value['name_prefix'].' '.$value['name'].' '.$value['lastname'].'</td>';                                                            
                        $t .= '<td>'.$text_status.'</td>';                                                            
                        $t .= '<td><a class="btn btn-xs btn-primary" href="'.base_url().'enquiry/enq_redirect/'.$value['enquiry_id'].'" >GO</a></td>';   
                        $t .= '</tr>';                                    
                    }
                    $t .= '</tbody>';                                    
                    $t .= '</table>';                
            }
            $res = json_encode(array('status'=>$status,'msg'=>$msg,'resp'=>$t));
		}else{	            
            $msg = '<span class="badge badge-success"  style ="background:green;padding:2px;"><i class ="fa fa-check"></i> Use '.$field.'</span>';            
            $status = 1;
			$new_enq='<span class="badge badge-success"  style ="background:green;padding:2px;"><i class ="fa fa-check"></i>Create new enquiry<a href="'.base_url().'enquiry/create?mdn='.$mob.'" class="btn btn-xs btn-success"  type="button"  href="javascript:void(0)">
                  
                </a> </span>';;
            $res = json_encode(array('status'=>$status,'msg'=>$msg,'new_enq'=>$new_enq));            
		}
        echo $res;
    ...
    public function test_test(){
        $this->enquiry_model->get_last_account_code();
    }
