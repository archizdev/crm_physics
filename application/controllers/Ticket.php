...
class Ticket extends CI_Controller
{
	...
	public function ticket_load_data(){
		...
		foreach ($res as $point) {
			...
			if ($showall or in_array(16, $acolarr)) {
				$tck_status = $point->status_name == '' ? 'Open' : $point->status_name;		

				if(isset($_SESSION['ticket_filters_sess']['followup']) && $_SESSION['ticket_filters_sess']['followup']=='1'){
					if($point->status_name == ''){
						$tck_sts = $this->get_last_ticket_status($point->conv_id,$point->tck_id);					
						$sub[] = $tck_sts;//$point->current_status_name
	...
	public function get_last_ticket_status($id,$tck_id){
		$res = $this->db->query("SELECT tbl_ticket_status.status_name,tbl_ticket_conv.id FROM `tbl_ticket_conv` inner join tbl_ticket_status on tbl_ticket_status.id=tbl_ticket_conv.ticket_status WHERE `tck_id`=$tck_id and tbl_ticket_conv.id<$id and `ticket_status` !='' ORDER by tbl_ticket_conv.id DESC LIMIT 1
		")->row_array();
		if(!empty($res['status_name'])){
