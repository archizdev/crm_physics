...
class Dashboard extends CI_Controller {
    ...
    public function fb_page(){ 
        ...
        if(empty($res)){
         ...
         $this->db->set('res',json_encode($this->input->post('all_data')));
       ...
       //$this->db->where('s!=',3);
       $this->db->where('id between 19904 and 19934');
       //$this->db->limit(5); 
					   ...
					   if($user_data->user_permissions==151){
                        ...
                        if($user_data->companey_id==57){
                            ...
                            if(user_access(223)){ 
                                ...
                                $this->telephony_model->create_session();
                            }
                            $res = array('status'=>true,'message'=>'Successfully Logged In');                       
                        }
                   }
...
if (!empty($enquiry_separation)) {
    ...
    public function logout() {
      ...
      if(user_access(223)){
        $this->load->model('telephony_model');
        $this->telephony_model->end_session();
      }
    ...
    public function selected_country() {
     ...
     public function forgot_password() {
           ...
           /*  if(empty($email_row) && (!empty($data->companey_id)) != 81){ 
               echo 'gg';// echo "4";die;                
            }else{ */
                if(is_numeric($email) == 1)
                {
                  expirePreviousOTP(!empty($data->pk_i_admin_id));
                  ...
                  //$this->Message_models->smssend($phone,$message,!empty($data->companey_id),!empty($data->pk_i_admin_id));
                  echo "99";
                }
                else
                {
                   ...
                   $email_data['url'] = $this->config->base_url()."change-password/" . base64_encode(!empty($data->pk_i_admin_id));
                   //$this->load->library('email');
    ...
    public function verifyOTP()
    {
      ...
      if(!empty($data))
      { //print_r($data);
        $getOtp = $this->db->select('*')->from('tbl_otp')->where('user_id',$data->pk_i_admin_id)->where('status',1)->order_by('id','DESC')->get()->row();
        //  print_r($getOtp);
		  if($otp == $getOtp->otp)
        {
