   ...
   public function get_call_status($uid=''){
    ...
    public function get_in_status($uid=''){
        ...
        if(!empty($call_json['event']) && $call_json['event'] ==1){
            $inbound = 1;
        }else if(!empty($call_json['event']) && $call_json['event'] ==2){
            $inbound = 3;
        }
        if(in_array($user_id,$array_users)){
         echo $inbound;
    ...
    public function click_to_dial($phone='',$integration_type=''){        
        ...
        $support_user_id = $this->session->telephony_agent_id;
            if(user_access(222)){
                ...
                curl_close($curl);
                echo $response;
        }else if(user_access(224)) {
            $this->load->model('telephony_model');
            $this->telephony_model->click_to_dial($phone);
        }else if(user_access(223)){
            $this->load->model('telephony_model');
            $this->telephony_model->click_to_dial_cloud_connect($phone);
        }
    }
    ...
    function get_agent_id(){
        ...
        curl_setopt_array($curl, array(
        CURLOPT_URL => 'https://crm4.cloud-connect.in/CCC_api/v1.4/getAgentId',
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 0,
    ...
    function get_campaign(){
        ...
        curl_setopt_array($curl, array(
        CURLOPT_URL => 'https://crm4.cloud-connect.in/CCC_api/v1.4/getCampaignDetails',
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 0,
