	...
	public function __construct()
	{
 	...
 	public function enquiry_extra_field($comp_id,$tab_id=1,$form_for=0){ // add extra fileds to enquiry form 		
 		...
 		if($form_for==2){
 			...
 			echo $this->load->view('forms/custom_qr_form',$data,true);	
		}else{
...
// if($comp_id==99){
// $this->db->where('tbl_input.fld_type',$for);
// }		
 		...
 		if(empty($data['basic_fields']))
 		{
	 			...
	 			foreach ($basic_fields as $key => $value)
				{
					...
					{
						$this->db->insert('qr_fileds_basic',array('comp_id'=>$comp_id,'field_id'=>$value['id'])); 
					}else
					{
	...
	public function enquiry_save_custom_field($comp_id){	
    	...
    	if ($this->form_validation->run() == TRUE) {
			...
			$fld_type = $this->input->post('fld_type');
	...
	public function get_basic_field_by_process(){		
	    ...
	    else
	    {
	        ...
	        $data['product_category'] = $this->product_model->get_category();
	       // $data['product_subcategory'] = $this->product_model->productcountry();
	...
	public function tabs(){
		...
		if($this->session->user_right != 1){
			$data['content'] = $this->load->view('forms/tab_list_company', $data, true);
		}else{
	...
	{	
		...
		$this->db->where("FIND_IN_SET($comp_id,comp_id)>",0);
		$tab_row = $this->db->get('forms')->row_array();
		if(!empty($tab_row)){
			$comp_ids = explode(',',$tab_row['comp_id']);
			if (($key = array_search($comp_id, $comp_ids)) !== false) {
				unset($comp_ids[$key]);
			}
			$comp_ids = implode(',',$comp_ids);
			$this->db->set('comp_id',$comp_ids);
			$this->db->where('id',$tab_id);
	...
	public function add_form(){
        $this->session->set_userdata('redirect', current_url() . '/'. $this->input->server( 'QUERY_STRING' ));
         $data['comp_id'] = $this->session->companey_id;
         $data['content'] = $this->load->view('forms/custom-form-page', $data, true);
        $this->load->view('layout/main_wrapper', $data);
    }
}