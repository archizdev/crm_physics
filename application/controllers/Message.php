		...
		if ($this->input->post('mesge_type') == 1) {
			...
			if (!empty($move_enquiry) && is_array($move_enquiry)) {
				foreach ($move_enquiry as $key) {
					$enq = $this->enquiry_model->enquiry_by_id($key);
					$phone = '91' . $enq->phone;
					$integration_id = $this->Message_models->get_integration_id_by_rule($key, 1);
					$this->Message_models->sendwhatsapp($phone, $message, $comp_id, $user_id, $integration_id);
					$media_url = '';

					if ($template_row['media']) {
						$media_url	=	$template_row['media'];
						$this->Message_models->sendwhatsapp($phone, base_url() . $media_url, $comp_id, $user_id, $integration_id);
					}
				...
				if ($schedule == 1) {
					$integration_id = $this->Message_models->get_integration_id_by_rule($move_enquiry, 1);
					$this->Message_models->sendwhatsapp($phone, $message, $comp_id, $user_id, $integration_id);
					$media_url = '';
					if ($template_row['media']) {
						$media_url = $template_row['media'];
			...
			if (!empty($move_enquiry)) {
				...
				if (is_array($move_enquiry)) {
					...
					foreach ($move_enquiry as $key) {
						...
						$integration_id = $this->Message_models->get_integration_id_by_rule($key, 3);
						$this->db->where('comp_id', $this->session->companey_id);
						if ($integration_id) {
						...
						if($email_row['type'] == 1){
							...
							if (!empty($rows->media)) {
								$this->email->attach($rows->media);
								$media_url = $rows->media;
							}
							$send_mail = $this->email->send();
						}else if($email_row['type'] == 2){
							$to_name = $enq->name_prefix.' '.$enq->name.' '.$enq->lastname;
							$api_config = array(
								'sender_email'	=>	$email_row['smtp_user'],
								'sender_name'	=>	$email_row['protocol'],
								'end_point'		=>	$email_row['smtp_host'],								
								'api_key'		=>	$email_row['smtp_pass'],
								'msg'			=>	$message,
								'subject'		=>	$email_subject,
								'attachment'	=>	'',
								'to'			=> array(array('name'=>$to_name,'email'=>$to)),
								'cc'			=>	'',
								'bcc'			=>	'',
							);
							$this->Message_models->send_mail_api($api_config);
							$send_mail = 1;
						}
						if ($send_mail) {
							echo "Mail sent successfully";
							$save_message = ['message' => $message, 'media' => $media_url];
							$saveMsgTimelineId = $this->Message_models->AddMsgtimlineEnquiry($enq->Enquery_id, $save_message, $user_id, $email_subject, 'Send Mail');
						} else {
					...
					// $to=$enq->email;
					$integration_id = $this->Message_models->get_integration_id_by_rule($move_enquiry, 3);
					$this->db->where('comp_id', $this->session->companey_id);
					if ($integration_id) {
						$this->db->where('id', $integration_id);
					}
			...
			if (!empty($this->input->post('mobile'))) {
				...
				if ($schedule == 1) {
					...
					$integration_id = $this->Message_models->get_integration_id_by_rule($move_enquiry, 2);
					$this->Message_models->smssend($phone, $message, $comp_id, $user_id, $integration_id, $templates_id);
					echo "Message sent successfully";
					//only for ticket
					if ($msg_from == 'ticket') {
						//timeline add
				...
				if (!empty($move_enquiry)) {
					foreach ($move_enquiry as $key) {
						...
						$message  = str_replace('@visiting_card', base_url($user_meta['visiting_card']), $message);
						// if visiting card send end
						$integration_id = $this->Message_models->get_integration_id_by_rule($key, 2);
