...
class Enquiry extends REST_Controller {
   ...
   // $this->lang->load('notifications_lang', 'english');   
    ...
    function phone_check($phone){
      if(user_access('A62',$this->input->post('user_id'))){ // duplicate allowed right
        return true;
      }
        ...
        if(!$product_id){  
          ...
          if($this->input->post('company_id') == 81 && !empty($this->input->post('email'))){
          ...
          $query = $this->db->query("select enquiry_id,Enquery_id,drop_status,phone from enquiry where product_id=$product_id AND $where");
            ...
            // $this->db->where('comp_id',$this->input->post('company_id'));
            // $query = $this->db->query("select enquiry_id,Enquery_id,drop_status,phone from enquiry where product_id=$product_id AND phone=$phone");
            ...
            if ($query->num_rows()>0) {
              if($this->input->post('company_id') == 81){
                $enq_row = $query->row_array();
                if($enq_row['drop_status'] > 0){
                  $this->db->where('comp_id',$this->input->post('company_id'));
                  $this->db->where('enquiry_id',$enq_row['enquiry_id']);
                  $this->db->set('drop_status',0);
                  $this->db->set('status',1);
                  $this->db->update('enquiry');
                  $this->Leads_Model->add_comment_for_events('Dropped enquiry activeted', $enq_row['Enquery_id'],'',$this->input->post('user_id'));
                  
                  $this->form_validation->set_message('phone_check', 'Your current data is activeted');
                  return false;
                }
              }
    ...
    { 	
      		...
      		if($this->form_validation->run() === true) 
          {
            ...
            if($comp_id == 87){
              $account = $this->input->post('mobileno', true);
              //$account = $this->enquiry_model->get_company_id($mobile,$comp_id,$process_id);
              $account = substr($this->input->post('fname'), 0, 4).substr($account, -6);
            }else{
              $account = $this->input->post('org_name');
            }
           // echo $account; exit;
...
public function get_enq_list_post(){
	...
	public function career_ex_contact_form_post($process_id=175){
      ...
      exit;
		...
		if ($this->form_validation->run() == true) {	
			...
			curl_setopt_array($curl, array(
			  	...
			  	CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
			  	CURLOPT_CUSTOMREQUEST => "POST",
			  	CURLOPT_POSTFIELDS => array('fname' => $fname,'lastname' => $last_name,'email' => $email,'mobileno' => $mobile,'company_id' => '81','process_id' => $process_id,'product_id' => $product_id,'enquiry_source'=>244,'enquiry'=>'','user_id' => '511','address'=>$address,'4023'=>$join_date),
			  	CURLOPT_HTTPHEADER => array(
			    	"Cookie: ci_session=3ba7d4lq4alv2pgpq3sc8t2ojrh41s04"
			  	),
			));
