...
class Report extends CI_Controller
{
  ...
  public function view($id)
  {
    ...
    $from = $this->session->set_userdata('from1', $this->input->post("from"));
    $to =  $this->session->set_userdata('to1', $this->input->post("to"));
  ...
  public function all_report_filterdata()
  {
   ...
   // echo $this->db->last_query();
    ...
    foreach ($rep_details as  $repdetails) {
      ...
      if (in_array('Call Duration', $this->session->userdata('post_report_columns'))) {
        $row[] = (!empty($repdetails->dur)) ? $repdetails->dur : 'NA';
      }
      if (in_array('Call Status', $this->session->userdata('post_report_columns'))) {
        $row[] = (!empty($repdetails->call_status)) ? (($repdetails->call_status==1)?'Connected':'Missed') : 'NA';
      }
      if (in_array('Call Id', $this->session->userdata('post_report_columns'))) {
        $row[] = (!empty($repdetails->call_id)) ? $repdetails->call_id : 'NA';
      }
      if (in_array('Agent Id', $this->session->userdata('post_report_columns'))) {
        $row[] = (!empty($repdetails->agent_id)) ? $repdetails->agent_id : 'NA';
      }
      if (in_array('Call Type', $this->session->userdata('post_report_columns'))) {
        $row[] = (!empty($repdetails->event_type)) ? (($repdetails->event_type==1)?'Incoming':'Outgoing') : 'NA';
      }
      // if (in_array('Call Recording', $this->session->userdata('post_report_columns'))) {
      //   $row[] = (!empty($repdetails->Enquery_id)) ? $repdetails->Enquery_id : 'NA';
      // }
