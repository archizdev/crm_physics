...
echo json_encode($res);die;
  ...
  public function send_sales_view($id){
   ...
   session_unset();   
    ...
    if(!empty($_GET['region'])){
      ...
      $get_user = $this->db->query("SELECT GROUP_CONCAT(pk_i_admin_id) as ids FROM tbl_admin WHERE b_status=1 AND FIND_IN_SET(141,process) > 0 AND sales_region = ".$region_id."")->result_array();
    ...
    if ($report_row['type'] == '1') {
      ...
      //echo $this->db->last_query();
      $data['emp_region'] = $this->db->get_where('sales_region')->result_array();
      $this->load->view('reports/send_sales_view', $data);
  ...
  public function get_last_month_data(){
    ...
    $urs_arr_str_1 = '';
    foreach($sales_region as $key => $region){
      $get_user = $this->db->get_where('tbl_admin',array('b_status'=>1,'sales_region' => $region['region_id']))->result_array();
      $urs_arr = array();
  ...
  public function get_last_month_avg_data(){
    ...
    $grand_total = array();
    foreach($sales_region as $key => $region){
      $get_user = $this->db->where(array('b_status'=>1,'sales_region' => $region['region_id']))->from('tbl_admin')->count_all_results();
      array_push($total_users,$get_user);
      $html .='<tr>
                 <th style="text-align:center;font-size:8px;">'.$region['name'].'</th>
                 <th style="text-align:center;font-size:8px;">'.$get_user.'</th>';
  ...
  public function all_call_log_report_filterdata()
  {
    ...
    $all_users = $this->db->where(array('b_status'=>1))->from('tbl_admin')->count_all_results();
