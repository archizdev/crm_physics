...
class Enquiry extends CI_Controller
{
    ...
    public function cloud_connect_integration(){
        $data = array();
        $data['content'] = $this->load->view('cloud_connect_integration',$data,true);
    ...
    public function bulk_dial_myoperator(){
        //print_r($_POST);
        ...
        if(!empty($_POST['enquiry_id'])){
            ...
            $enq_rows = $this->db->get('enquiry')->result_array();
            if(!empty($enq_rows)){
                foreach($enq_rows as $key=>$value){
                    $phn  = '+91'.$value['phone'];
                    $this->Telephony_model->click_to_dial($phn);
                }
            }            
            //echo count($enquiry_id).' call initiated';
        }else{
    ...
    {
        if(user_access('A62')){ // duplicate allowed right
            return true;
        }
    ...
    public function qutation_pdf(){
        $this->load->view('qutation_html');
    }
    ...
    function activityTimeline($for=0)
    {
        ...
        foreach ($comment_details as $comments) {
            ...
            $call_html  = '';
            if(!empty($comments->call_id)){
                $call_id = $comments->call_id;
                $call_row = $this->db->where('call_id',$call_id)->get('tbl_col_log2')->row_array();
                if(!empty($call_row)){
                    $call_json = json_decode($call_row['json_data'],true);
                    if(!empty($call_json)){
                        
                        $call_type_ = ($call_row['event_type']=="1")?"Inbound":"Outbound";
                        if(empty($call_json["_fu"])){
                            $rec = 'javascript:void(0)';
                        }else{
                            $rec = $call_json["_fu"].".mp3";
                        }
                        $call_html .= '<br><span style="font-size:14px;"><a target="_black" href="'.$rec.'">'
                        .''.$call_row["dur"].'</a> '. $call_type_ .'</span>';
                    }
                }
            }
        ...
        if ($assigned_user != NULL ) {
          ...
          $countdassigned=$dassigned->num_rows();
           ...
           $userlName=$asdata->last_name;
            ...
            //echo $comments->comment_msg.'<br>';
            if ($comments->comment_msg == 'Stage Updated' && $for == 0) {
         $html .= '<li>
            ...
            } else if ($comments->comment_msg == 'Enquiry moved' && $for == 0) {
                //echo 'hello';
                $html .= '    <li>
            ...
            } else if (($comments->comment_msg == 'Move to '.display('lead').'') && $for == 0 ) {
                $html .= '<li>
            ...
            } else if ($comments->comment_msg == display('enquery_create') && $for == 0) {
                $html .= '   <li>
            ...
            } else if ($comments->comment_msg == 'Enquiry dropped' || $comments->comment_msg == 'Lead dropped' || $comments->comment_msg == 'Client dropped' && $for == 0) {
                $html .= '<li>
                  <div class="cbp_tmicon cbp_tmicon-phone"  style="background:#d68910;"></div>
                  <div class="cbp_tmlabel"  style="background:#95a5a6;">
                    <span style="font-weight:900;font-size:13px;">' . ucfirst($comments->comment_msg) . ' </span></br>
            ...
            else if($comments->comment_msg=='Visit Added' && $for == 0)
            {
               
               $js_call = 'onclick="getTimelinestatus('.$comments->comm_id.','.$comments->stage_id.');" data-target="#timelineshow_d"';
             ...
             else 
             {
                ...
                if(in_array($comments->comment_msg,array('Send Mail','Send SMS','Send Whatsapp')))
                {
                    ...
                    //echo $comments->comment_msg;
                    //$html .= 'hi';
                }
                if((!empty($js_call) && (( $for == 1 && $comments->comment_msg=='Send Mail') || ($for == 2 && $comments->comment_msg=='Send SMS') || ($for == 3 && $comments->comment_msg=='Send Whatsapp'))) || ($for == 0 && $js_call=='')){
                    $html .= '  <li>
                        <div class="cbp_tmicon cbp_tmicon-phone"  style=""></div>
                        <div class="cbp_tmlabel"  '.$js_call.' style="background:#95a5a6;" data-toggle="modal" >
                        <span style="font-weight:900;font-size:13px;">' . ucfirst($comments->comment_msg) . ' </span>';
                        if(strlen(trim($comments->remark)))
                        $html.='</br>
                        <span style="font-weight:900;font-size:10px;">' . ucfirst($comments->remark) . '</span>';
                    ...
                    if ($assigned_user != NULL) {
                        ...
                        $html .= '<br>
                        <div style="font-size:11px; line-height:16px;">Assigned To: <strong> '.$userFName.' '.$userlName.' </strong></div>';
                        }
                    }
                    }
    ...
    if($log->num_rows()==1){
        ...
        foreach ($log->result() as $key => $value) {
            ...
            $details1 = '
               ...
               <div id="timelinesdata-data">'.$message.'</div>
               </div>
               <div class="modal-footer">
                 <div class="row">
                   <div class="col-md-4">
 ...
 public function update_info_status()
 {
    ...
    {   
        ...
        $vid=    $this->Client_Model->add_visit($data);
            $this->Leads_Model->add_comment_for_events('Visit Added',$res->Enquery_id,$vid);
    ...
    public function visit_load_data()
    {
        ...
        foreach ($result as $res)
        {
            ...
            $udata = $this->user_model->read_by_id($res->user_id);
            $x = $this->db->where('id',$udata->discount_id)->get('discount_matrix')->row();
    ...
    public function lead_aging_rule_exec($comp_id,$lid){     
       ...
       if(!empty($rules)){
        ...
        //$this->save_cron_logs(8,'lead_aging_rule_exec');
       }       
    }

    public function save_cron_logs($type,$cron_name){
        $insert_data = array(
            'type'          => $type,
            'added_date'    => date('Y-m-d H:i:s'),
            'cron_name'     => $cron_name
        );
        $this->db->insert('tbl_cron_logs',$insert_data);
    }
    ...
    function upload_fb_data()
    {
        ini_set('max_execution_time', '-1');
        $filename = "fbdata_" . date('d-m-Y_H_i_s');
        $config = array(
            'upload_path' => $_SERVER["DOCUMENT_ROOT"] . "/assets/enquiry",
            'allowed_types' => "text/plain|text/csv|csv",
            'remove_spaces' => TRUE,
        ...
        if ($this->upload->do_upload('img_file')) {
            ...
            while (($filesop = fgetcsv($handle, 2000, ",")) !== false) {
                ...
                if ($count == 1) {
                    ...
                    $this->db->where('country_name',$filesop['5']);
                    $crow = $this->db->get('tbl_product_country')->row_array();
                    if(!empty($crow['id'])){
                        $k = array(
                            'from_id'=>$filesop['0'],
                            'from_name'=>$filesop['1'],
                            'add_set_name'=>$filesop['2'],
                            'compaign_name'=>$filesop['3'],
                            'add_name'=>$filesop['4'],
                            'course_name'=>$crow['id'],
                            'comp_id'=>81,
                            'created_by'=>511
            ...
            // print_r($res);      
            // echo "</pre>"; 
            $this->db->insert_batch('fb_from_details',$res);

            if ($record > 0) {
                $res = 'Record(' . $record . ') inserted';
                unlink($filePath);
                $this->session->set_flashdata('message', "File Uploaded successfully." . $res);
                redirect(base_url() . 'enquiry/upload_fb_data_csv');
            } else {
                $res = 'No  record Found !';
                unlink($filePath);
                $this->session->set_flashdata('message', "File Uploaded successfully." . $res);
            ...
            if ($failed_record) {
                unlink($filePath);
                $res .= ' (' . $failed_record . ') record course not matched';
                echo "<pre>";
                print_r($failed_record_arr);
                echo "</pre>";
            }
        } else {
            $this->session->set_flashdata('exception', $this->upload->display_errors());
    ...
    public function upload_fb_data_csv(){
        $data['title'] =  'Upload Facebook Data';
        $data['content'] = $this->load->view('upload_fb_data',$data,true);
        $this->load->view('layout/main_wrapper',$data);
    }

    public function get_comp_code($comp_name){
        $return = '';
        $this->db->where('comp_id',$this->session->companey_id);
        $this->db->where('company_name',$comp_name);
        $this->db->order_by('id','desc');
        $res = $this->db->get('tbl_company')->row_array();
        if(!empty($res)){
            $return  = $res['comp_code'];
        }
        echo $return;
    }

    public function get_comp_code_by_mobile($mobile){
        $response = array('name'=>'','code'=>'');
        $return = '';
        $this->db->where('phone',$mobile);
        $this->db->where('comp_id',$this->session->companey_id);
        $res    =   $this->db->get('enquiry')->row_array();
        if(!empty($res['company'])){
            $this->db->where('comp_id',$this->session->companey_id);
            $this->db->where('id',$res['company']);
            $comp_row = $this->db->get('tbl_company')->row_array();   
            $response = array('name'=>$comp_row['company_name'],'code'=>$comp_row['comp_code'],'enq_res'=>$res);
        }
    ...
    public function mark_tag(){        
        ...
        if($this->form_validation->run() == true){
            ...
            foreach ($enq as $key => $value) {
                $row_arr = $this->db->where('enq_id',$value)->get('enquiry_tags')->row_array();
                if(!empty($row_arr)){
                    $tags = explode(',',$row_arr['tag_ids']);
                    if(!empty($unchecked)){
                        $tags = array_diff($tags,$unchecked);
                    }
                    if(!empty($checked)){
                        $tags = array_merge($tags,$checked);
                    }
                    if(!empty($tags)){
                        $tags = implode(',',$tags);
                    }
                    ...
                    if($checked){
                        ...
                        $this->db->insert('enquiry_tags',array('comp_id'=>$this->session->companey_id,'enq_id'=>$value,'tag_ids'=>$checked));
                    }
                }
            }
    ...
    public function tags_checkboxs(){
        $enquiry_ids = $this->input->post('enquiry_id[]');        
        $enquiry_ids = implode(',',$enquiry_ids);
        $result = $this->db->query("SELECT GROUP_CONCAT(`enquiry_tags`.`tag_ids` SEPARATOR ',') as tags FROM enquiry_tags INNER JOIN `enquiry` on `enquiry_tags`.`enq_id`=`enquiry`.`enquiry_id` where `enquiry_tags`.`enq_id` IN (".$enquiry_ids.")")->row_array();

        $t = explode(',',$result['tags']);
        $t = array_unique($t);        
	    $tags = $this->enquiry_model->get_tags();
        if(!empty($tags)){
            foreach ($tags as $tag) { ?>
            <div class="col-md-6">                        
                <input name="tags[]" type="checkbox" value="<?=$tag['id']?>" <?php if(in_array($tag['id'],$t)){ echo "class='make-indeterminate'"; } ?> > <?php echo $tag['title']; ?>
            </div>
            <?php
            }
        }
