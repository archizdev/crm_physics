...
if($root=='https://student.spaceinternationals.com'){  ?>
    ...
    <?php
      ...
      if(in_array(220, $module) || in_array(222,$module)){
...
if($root=='https://student.spaceinternationals.com'){  ?>
                ...
                <ul class="sidebar-menu">
                    ...
                    <li
                        ...
                        <ul
                            ...
                            <?php
                            if(!in_array(440,$module)){                            
                            ?>                            
                            <li class="<?php echo (in_array($segment2,array('category')) ?"active":'') ?>">
                                <a href="<?php echo base_url() ?>product/category">
                                    <?php echo display('product_category'); ?></a>
                            </li>
                            <?php }
                            }
                            ?>
            ...
            <?php  if($this->session->menu==1){ ?></br><p style="color:#fff;font-size:9px;margin-left:-12px;padding-top:10px;"><?php echo display('Inventory'); ?></p> <?php } ?>
                  ...
                  if ($this->session->user_right == 1 ) { ?>
                    ...
                    <li class="treeview <?php echo (($segment1 == "cron") ? "active" : null) ?>">
                        <a href="<?php echo base_url("cron") ?>">
                            <i class="fa fa-cogs icon-class"></i>
                            &nbsp;<?php echo 'Crons' ?>
                            <?php  if($this->session->menu==1){ ?></br>
                            <p style="color:#fff;font-size:9px;margin-left:-12px;padding-top:10px;">
                                <?php echo 'Crons' ?></p> <?php } ?>
                        </a>
                    </li>
                  ...
                  if ($this->session->user_right != 1 && in_array(1080,$module)) { ?>
                    <li class="treeview <?php echo (($segment1 == "form") ? "active" : null) ?>">
                        <a href="<?php echo base_url("form/form/add_form") ?>">
                            <i class="fa fa-cogs icon-class"></i>
                            &nbsp;<?php echo 'Form Builder' ?>
                            <?php  if($this->session->menu==1){ ?></br>
                            <p style="color:#fff;font-size:9px;margin-left:-12px;padding-top:10px;">
                                <?php echo 'Form Builder' ?></p> <?php } ?>
                        </a>
                    </li>
                    <?php
                  }
 ...
 -->
                ...
                ?>
                    ...
                    <li class="treeview <?php echo (($segment1 == "setting") ? "active" : null) ?>"
                        style="<?php if(in_array('setting1',$module)){ echo 'display:block;';}else{echo 'display:none;';}?>">
                        <a href="<?php echo base_url("setting") ?>"><i class="fa fa-cogs icon-class"></i>
                            &nbsp;<?php echo "Application Settings"; ?>
                            <?php  if($this->session->menu==1){ ?></br>
                            <p style="color:#fff;font-size:9px;margin-left:-12px;padding-top:10px;">
                                <?php echo "Application Settings"; ?></p> <?php } ?>
                        </a>
                    </li>
...
// if($this->session->companey_id=='65')
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                if (data == 1) {
                                    ...
                                    Swal.fire({
                                        icon: 'info',
                                        html: '<strong>Outgoing call on this number.<a href="<?=base_url()?>telephony/forword_to/' +
                                            phone_n + '">' + disp_no +
                                            '</a></strong><br><a class="btn btn-info" href="<?=base_url()?>telephony/forword_to/' +
                                            phone_n + '">Go</a>',
                                        showCancelButton: false,
    ...
    $("#disposition_save_btn").on('click', function(e) {
        ...
        var uid = "<?=$this->session->user_id?>";
        console.info("UID:"+uid+" subj:"+subject+" enqid:"+enq_id+" task:"+task_date+" time:"+time);   
        id = writeUserData(uid, subject, enq_id, task_date, time);
        //console.log(id);
...
<?php
if(user_access(223) && !empty($this->session->cloud_connect_session_id)){
    ?>
    <div style="position:fixed;bottom:48px;right: 10px;">
        <button onclick="cloud_connect_show()" style="background-color:#31B0D5;color:white;padding:10px 20px;border-radius:4px;border-color:#46b8da;"><i class='fa fa-phone'></i></button>
    </div>
    <iframe id='cloud_connet_iframe' allow="microphone" style="float: right;position: fixed;bottom: 0;left: 325px;width: 72%;height: 10%;" src="https://crm4.cloud-connect.in/wp_plugin/index.php?r=agent&session=<?=$this->session->cloud_connect_session_id?>" height="80" width="80%" title="Iframe Example"></iframe>
        <script>
        $("#cloud_connet_iframe").hide();
        function cloud_connect_show(){
            $("#cloud_connet_iframe").toggle();
        }
        </script>
    <?php
}
