		        ...
		        <div class="panel-body">
					   	...
					   	<div class="col-sm-3">					   	
					   		...
					   		<select class="form-control" id="rule" onchange="builder_fun(this.value)">
								...
								<option value="10" <?=(!empty($rule_data['type']) && $rule_data['type']==10)?'selected':''?>>Ticket move to sales</option>
								 ...
								 <option value="12" <?=(!empty($rule_data['type']) && $rule_data['type']==12)?'selected':''?>>Whatsapp Api Integration</option>

								 <option value="13" <?=(!empty($rule_data['type']) && $rule_data['type']==13)?'selected':''?>>Email Api Integration</option>

								 <option value="14" <?=(!empty($rule_data['type']) && $rule_data['type']==14)?'selected':''?>>SMS Api Integration</option>
				   ...
				   <div class="row">   
			           ...
			           <div id="builder"></div>
						...
						<div id="email_integration" class="action-section text-center row"> 
								<div class="col-md-3"></div>
								<div class="col-md-2">
			        				<label>Select Integration<i style="color: red;">*</i></label>									
								</div>
								<div class="col-md-2">
									<select name="email_integration" class='form-control'>
									<?php
									if(!empty($email_api_list)){
										foreach($email_api_list as $key=>$value){
											?>
											<option value="<?=$value['id']?>" <?php if(!empty($rule_data['rule_action']) AND $rule_data['rule_action']== $value['id']){ echo'selected'; } ?>><?=$value['name']?></option>
											<?php
										}
									}
									?>
						...
						<div id="sms_integration" class="action-section text-center row">
								<div class="col-md-3"></div>
								<div class="col-md-2">
									<label>Select Integration<i style="color: red;">*</i></label>									
								</div>
								<div class="col-md-2">
									<select name="sms_integration" class='form-control'>									
									<?php
									if(!empty($sms_api_list)){
										foreach($sms_api_list as $key=>$value){
											?>
											<option value="<?=$value->api_id?>" <?php if(!empty($rule_data['rule_action']) AND $rule_data['rule_action']== $value->api_id){ echo'selected'; } ?>><?=$value->api_name?></option>
											<?php
										}
									}
									?>
						...
						<div id="whatsapp_integration" class="action-section text-center row">
								<div class="col-md-3"></div>
								<div class="col-md-2">
									<label>Select Integration<i style="color: red;">*</i></label>									
								</div>
								<div class="col-md-2">
									<select name="whatsapp_integration" class='form-control'>
										<?php
										if(!empty($whatsapp_api_list)){
											foreach($whatsapp_api_list as $key=>$value){
												?>
												<option value="<?=$value->api_id?>" <?php if(!empty($rule_data['rule_action']) AND $rule_data['rule_action']== $value->api_id){ echo'selected'; } ?>><?=$value->api_name?></option>
												<?php
											}
...
function builder_fun(rule_type)
				...
				if((['1','2','3','4','6','7','11','12','13','14']).includes(rule_type))
				{
					if(rules_basic=='')
					{
						rules_basic = {				  
	...
	$(document).ready(function(){
		...
		$('#btn-set').on('click', function() {		  
		  ...
		  if (!$.isEmptyObject(result)) {
		  	...
		  	if (rule_type==1) {
		  		...
		  		var	action_value =	$("select[name='whatsapp_integration']").val();
		  	}else if (rule_type==13){
		  		var	action_value =	$("select[name='email_integration']").val();
		  	}else if (rule_type==14){
		  		var	action_value =	$("select[name='sms_integration']").val();
		  	}
		...
		function action_show(){
			...
			if (rule == 1) {			
				...
				$("#whatsapp_integration").show(1000);
				
			}else if (rule == 14) {				
				$("#sms_integration").show(1000);
				
			}else if (rule == 13) {				
				$("#email_integration").show(1000);
