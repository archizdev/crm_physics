...
class Enquiry_model extends CI_Model {
    ...
    {   
      ...
      if(!empty($company)){
        ...
        $client_code = $company->comp_code;
        $account_id = $company->id;
      }else{
        $prefix = get_sys_parameter('account_code_prefix','COMPANY_SETTING',$comp_id);
        if(!$prefix){
            $prefix = 'CUST';
        }
        $new_company = array(
        ...
        if($this->session->companey_id == 90){
            $cust_code = $this->get_last_account_code();
        }else{
            $cust_code = $prefix.$data['company'];
        }
        
        $this->db->where('tbl_company.comp_id',$comp_id);
        $this->db->where('tbl_company.id',$data['company']);
        $this->db->set('tbl_company.comp_code',$cust_code);
        $this->db->update('tbl_company');
        $client_code = $cust_code;
        $account_id = $data['company'];
      }
    }else{
        $client_code = 'CONTACT';
    }
   ...
   if(!empty($data['name'])){
        $contact_code = '';
        if(!empty($data['company'])){ 
            $this->db->where('account_id',$data['company']);
            ...
            if(!empty($crow['contact_code'])){
                ...
                $next_char++;                
                //$contact_code = $crow['contact_code'].$next_char;
                $contact_code = substr_replace($crow['contact_code'] ,$next_char,-1);

            }else{
                $contact_code = $client_code.'A';
            }
        }
        ...
        if(!empty($contact_code)){
            $this->db->where('tbl_client_contacts.cc_id',$insert_id);
            $this->db->set('contact_code',$contact_code);
            $this->db->update('tbl_client_contacts');
        }else{
            $this->db->where('tbl_client_contacts.cc_id',$insert_id);
            $this->db->set('contact_code','CONTACT'.$insert_id);
            $this->db->update('tbl_client_contacts');
        }
    ...
    public function get_company_id($mobile,$comp_id,$process_id){    
        ...
        if(!empty($res['company'])){
            ...
            $cust_code = $prefix.$insert_id;

            $this->db->where('tbl_company.comp_id',$comp_id);
            $this->db->where('tbl_company.id',$insert_id);
            $this->db->set('tbl_company.comp_code',$cust_code);
            $this->db->update('tbl_company');
    ...
    public function get_last_account_code(){
        $this->db->where('comp_id',$this->session->companey_id);        
        $this->db->where('comp_code!=','');        
        $this->db->order_by('id','desc');
        $res = $this->db->get('tbl_company')->row_array();
        if(!empty($res['comp_code'])){
            $comp_code =  $res['comp_code'];
            $comp_code = substr($comp_code, 4);
            $serial = $comp_code+1; 
            $comp_code = 'PKPL'.$serial;
        }else{
            $comp_code = 'PKPL0001';
        }
        return $comp_code;
...
/*----------------------- all Dropped Enquiry----------------------*/
    ...
    public function enquiry_by_id($enquiry_id) {
        return $this->db->select("*,enquiry.created_by as enq_created_by,enquiry.city_id as enquiry_city_id,enquiry_tags.tag_ids,enquiry.state_id as enquiry_state_id,enquiry.created_date,enquiry.status,enquiry.address,tbl_product_country.country_name,tbl_product_country.id as country_id,tbl_product.product_name,tbl_center.center_name,enquiry.lead_created_date")
                        ->from($this->table)
...
$cpny_id=$this->session->companey_id;
    ...
    {	
        ...
        // if($this->session->companey_id==99 && $this->session->user_id==3169){
        //     print_r($result3);
        //     echo $this->db->last_query();
        //     echo $enq_ut;
        // }
