...
public function all_course($course,$lvl,$length,$disc) {
    ...
    public function get_leadListDetailsby_id($leadid) {
        return $this->db->select("*,enquiry.name as first_name,enquiry.created_by as enq_created_by,enquiry.created_date,tbl_newdeal.bank,enquiry.country_id as enq_country,enquiry.state_id as enquiry_state_id,enquiry.city_id as enquiry_city_id,enquiry.address,enquiry.status,tbl_product_country.country_name,tbl_product_country.id as country_id,tbl_product.product_name,tbl_center.center_name,tbl_company.company_name,tbl_company.comp_code,tbl_category.id as product_category_id")
                        ->from('enquiry')
                        ->join('tbl_product_country', 'tbl_product_country.id=enquiry.enquiry_subsource', 'left')
                        ->join('tbl_category', 'tbl_product_country.category=tbl_category.id', 'left')
...
public function all_description($diesc) {
  ...
  public function get_leadsource_list_api($compno) {
    ...
    public function add_comment_for_events($conversation, $lead_id,$stage_code=0,$user_id=0) {
        ...
        if(user_access(224)){
            $enq_row = $this->db->select('phone')->where('comp_id',$comp_id)->where('Enquery_id',$lead_id)->get('enquiry')->row_array();
            $customer_no = $enq_row['phone'];
            $this->update_call_id_comment_tbl($customer_no,$insert_id);
        }
        return $insert_id;
    }
    public function update_call_id_comment_tbl($customer_no,$comt_id){
        $comp_id = $this->session->companey_id;        
        $this->db->select('uid');
        $this->db->where('status',0);
        $this->db->where('comp_id',$comp_id);
        $this->db->where('phone_number',$customer_no);        
        $call_row = $this->db->get('tbl_col_log')->row_array();
        
        $k = $this->db->last_query();
        $this->db->insert('test_log',array('res'=>$k));

        $call_id = $call_row['uid'];         
        //$this->db->where('comp_id',$comp_id);
        $this->db->where('comm_id',$comt_id);
        $this->db->set('call_id',$call_id);

        if($this->db->update('tbl_comment')){
            $this->db->where('uid',$call_id);
            $this->db->where('comp_id',$comp_id);            
    ...
    public function add_comment_for_events_api($conversation,$lead_id,$user_id=0) {
        ...
        $comp_id = 0;
        if(!empty($user_id)){
            $row_arr = $this->db->select('companey_id')->where('pk_i_admin_id',$user_id)->get('tbl_admin')->row_array();
            $comp_id = $row_arr['companey_id'];
        }
        $this->db->set('comp_id', $comp_id);        
