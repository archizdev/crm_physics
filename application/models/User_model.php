...
$company=$this->session->userdata('companey_id');
    ...
    public function get_input_basic_fields($comp_id,$field_for=0){
        ...
        {
            return $this->db->select('basic_fields.id,basic_fields.type,basic_fields.title,qr_fileds_basic.status,qr_fileds_basic.process_id,input_types.title as type_title')
                        ->from('basic_fields')
                        ->join('qr_fileds_basic','qr_fileds_basic.field_id=basic_fields.id','left')
                        ->join('input_types','input_types.id=basic_fields.type','inner')
                        ->where('basic_fields.field_for',$field_for)
                        ->group_by('basic_fields.id')

                        ->where('qr_fileds_basic.comp_id',$comp_id)
                        ->order_by('qr_fileds_basic.fld_order','ASC')
                        ->get()                        
                        ->result_array();
        }
        else
...
public function updateLoginToken($user_id,$token,$type=0)
{
