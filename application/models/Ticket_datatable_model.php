...
class Ticket_datatable_model extends CI_Model{
    ...
    function __construct() {
       ...
       $search_string = array();
        ...
        if($showall or in_array(16,$acolarr))
        {
            ...
            // }
            // if($followup){
            //     $search_string[] = " status2.current_status_name ";    
            // }
        }
    ...
    public function _get_datatables_query($postData){
        ...
        if($followup)
        {
            $sel_string[] = " conv.id as conv_id,conv.tck_id,conv.subj as tck_subject,for_conv.lead_stage_name as tck_stage,for_conv_desc.description as tck_sub_stage,conv.msg as tck_msg,status2.status_name as current_status_name,conv.send_date as activity_date";
        }
        ...
        {
         if($followup){
            $this->db->join("tbl_ticket_status status","conv.ticket_status=status.id","LEFT");
            $this->db->join("tbl_ticket_status status2","tck.ticket_status=status2.id","LEFT");
         }else{
...
$CHK = 0;
        ...
        if(!empty($ticket_status)){            
            ...
            if($CHK)
                $where .= 'AND';
            if(is_array($ticket_status)){
                if($followup){                    
                    $where .= " conv.ticket_status in(".implode(',',$ticket_status).")";
                }else{
                    $where .= " tck.ticket_status in(".implode(',',$ticket_status).")";
                }
            }else{
                if($followup){
                    $where .= " conv.ticket_status =  '".$ticket_status."'"; 
                }else{
