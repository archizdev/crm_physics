...
$(document).on("keyup", "#chk-mob-number", function(){  
    var base_url = $('body').attr('data-baseUrl');
    var proc  = $("input[name='product_filter[]']:checked").val();  
    var post_url = base_url+"client/checkmobile";       
    if (($(this).val().length)==10) {
      $.ajax({
        ...
        success: function(resp){      
          ...
          $("#is-avl-mobile").html(res.msg);            
          if (res.status == 0) {          
            Swal.fire({         
              icon: 'info',
              html:'<strong>Data already exist with current mobile no. in current process</strong><br>'+res.resp,
              showCancelButton: true,
              confirmButtonColor: '#3085d6',
              cancelButtonColor: '#d33',
              customClass: 'swal-wide',
              confirmButtonText: 'Create New with same mobile no!'          
            }).then((result) => {
              if (result.value) {                       
              }else{
              $("#chk-mob-number").val('');
      ...
      if (($(this).val().length)>0) {         
        $("#is-avl-mobile").html('<span class="badge badge-danger" style ="background:red;padding:2px;">Invalid mobile no !</span>');                 
      }else{
        $("#is-avl-mobile").html();
      }   
    } 
  ...
  $(document).on("keyup", "#chk-email-id", function(){  
    var base_url = $('body').attr('data-baseUrl');
    var proc  = $("input[name='product_filter[]']:checked").val();  
    var post_url = base_url+"client/checkmobile";       
    ...
    if (regex.test(email_str)) {
      $.ajax({
        ...
        success: function(resp){      
          var res = JSON.parse(resp);
          $("#is-avl-email").html(res.msg);            
          if (res.status == 0) {          
            Swal.fire({         
              icon: 'info',
              html:'<strong>Data already exist with current email id. in current process</strong><br>'+res.resp,
              showCancelButton: true,
              confirmButtonColor: '#3085d6',
              cancelButtonColor: '#d33',
              customClass: 'swal-wide',
              confirmButtonText: 'Create New with same email id!'          
            }).then((result) => {
              if (result.value) {                       
              }else{
              $("#chk-email-id").val('');   
      ...
      if (($(this).val().length)>0) {         
        $("#is-avl-email").html('<span class="badge badge-danger" style ="background:red;padding:2px;">Invalid email id !</span>');                 
      }else{
        $("#is-avl-email").html();
      }   
    } 
